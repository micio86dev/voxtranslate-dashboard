...
export interface VoipNumber {
  ...
  customer_monthly_usd?: string | null;
...
// --- Buying and keeping numbers (spec 0115) ----------------------------------

/** One number on offer. Prices are what the CUSTOMER pays; ours never cross the wire. */
export interface VoipNumberOffer {
  e164: string;
  country: string;
  kind: string;
  /** Decimal strings, not numbers: money does not travel as a float. */
  monthly: string;
  setup: string;
  currency: string;
  regulatory_requirement: string | null;
...
export function searchVoipNumbers(
  orgId: string,
  q: NumberSearchQuery,
): Promise<ApiResult<{ offers: VoipNumberOffer[] }>> {
  const params = new URLSearchParams({ country: q.country });
  if (q.areaCode) params.set('area_code', q.areaCode);
  if (q.kind) params.set('kind', q.kind);
  if (q.limit) params.set('limit', String(q.limit));
  return request(
    'GET',
    `/api/business/organizations/${orgId}/voip/numbers/search?${params.toString()}`,
  );
}
 ...
 * Buy one.
 *
 * `purchase_key` is generated by the caller and must survive a retry: the same key buys
 * the same number once. Without it a timeout turns into a second number the organisation
 * pays for monthly, for ever, without having asked.
 */
export function buyVoipNumber(
  orgId: string,
  body: { e164: string; country: string; area_code?: string; purchase_key: string },
): Promise<
  ApiResult<{ id: string; e164: string; status: string; monthly: string; setup: string }>
> {
  return request('POST', `/api/business/organizations/${orgId}/voip/numbers`, body);
}

export function verifyVoipNumber(
  orgId: string,
  numberId: string,
): Promise<ApiResult<{ verification_status: string; method: string; code: string | null }>> {
  return request('POST', `/api/business/organizations/${orgId}/voip/numbers/${numberId}/verify`);
}

export function checkVoipVerification(
  orgId: string,
  numberId: string,
): Promise<ApiResult<{ verification_status: string }>> {
  return request(
    'POST',
    `/api/business/organizations/${orgId}/voip/numbers/${numberId}/verify/check`,
  );
}
